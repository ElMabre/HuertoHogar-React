...
function ProductCard({ product }) {
  ...
  // Reemplaza la función handleAddToCart anterior con esta:
  const handleAddToCart = () => {
    if (product && product.stock > 0) {
      addToCart(product.id, 1); // Llama a la función del contexto con ID y cantidad 1
    ...
    <Card className="h-100 shadow-sm">
      ...
      <Link to={`/producto/${product.id}`} style={{ textDecoration: "none" }}>
        <Card.Img
          ...
          } // Usa imagen del producto o placeholder
          alt={product.nombre}
          style={{
            height: "200px",
            objectFit: "contain",
            background: "#fff",
            padding: "1rem",
          }} // Estilos similares al CSS original
        />
      </Link>
      <Card.Body className="d-flex flex-column">
        {" "}
        {/* Flex column para empujar botones abajo */}
        ...
        {/* Origen (si existe) */}
        {product.origen && (
          <small className="text-muted d-block mt-1 mb-3">
